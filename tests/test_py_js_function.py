...
if TYPE_CHECKING:
    from py_mini_racer import AsyncJSFunction
...
def test_basic() -> None:
    ...
    async def define_and_use_function(mr: AsyncMiniRacer) -> None:
        async with mr.wrap_py_function(append) as jsfunc:
            # "Install" our JS function on the global "this" object:
            await cast("AsyncJSFunction", await mr.eval("x => this.func = x"))(jsfunc)

            # Call our function a couple times, through JS:
            assert (
                await cast("AsyncJSPromise", await mr.eval("this.func(42)")) == "foobar"
            )
            assert (
                await cast("AsyncJSPromise", await mr.eval('this.func("blah")'))
                == "foobar"
            )
...
def test_exception() -> None:
    ...
    async def run() -> None:
        async with async_mini_racer() as mr:
            async with mr.wrap_py_function(append) as jsfunc:
                # "Install" our JS function on the global "this" object:
                await cast("AsyncJSFunction", await mr.eval("x => this.func = x"))(
                    jsfunc
                )

                with pytest.raises(JSPromiseError) as exc_info:
                    await cast("AsyncJSPromise", await mr.eval("this.func(42)"))
    ...
    async def run() -> None:
        async with async_mini_racer() as mr:
            async with mr.wrap_py_function(append) as jsfunc:
                ...
                pending = [
                    cast("AsyncJSPromise", await mr.eval("this.func(42)"))
                    for _ in range(100)
                ]
...
def test_call_on_exit() -> None:
    ...
    async def run() -> None:
        ...
        async with async_mini_racer() as mr, mr.wrap_py_function(append) as jsfunc:
            # "Install" our JS function on the global "this" object:
            await cast("AsyncJSFunction", await mr.eval("x => this.func = x"))(jsfunc)

            # Note: we don't await the promise, meaning we just start a call and never
            # finish it:
            assert isinstance(await mr.eval("this.func(42)"), AsyncJSPromise)
